...
-- Domain is a function because we need to know what is on the stack before we append  to it

-- Left corresponds to Int (From the Either) and Right corresponds to Bool
-- s' means the rest of the stack
-- 7. Define the semantics of a StackLang command (ignore If at first).
cmd :: Cmd -> Domain
cmd (PushN i)     = \s -> Just (Left i : s) --append left i  onto the stack
