...
--Takes a list and return a tree
encodeList :: [a] -> Tree a
encodeList []     = End
...
mapTree _ End            = End
mapTree f (Node v xl xr) = Node (f v) (mapTree f xl) (mapTree f xr)
...
valueAt [] (Node f l r)     = Just f
valueAt _  End              = Nothing
valueAt (x:xs) (Node f l r) = case x of
                              L -> valueAt xs l
                              R -> valueAt xs r
